...
export default function AdminDetailsPage() {
  ...
  useEffect(() => {
    // Redirect to create page if the ID is "new"
    if (adminId === "new") {
      router.replace("/dashboard/admins/create");
      return;
    }
    ...
    const fetchAdminDetails = async () => {
      try {
        ...
        try {
          const sessionsResponse = await fetch(`/api/admins/${adminId}/sessions?limit=1`);
          if (sessionsResponse.ok) {
            const sessionsData = await sessionsResponse.json();
            if (sessionsData.totalCount !== undefined && data) {
              setAdmin({
                ...data,
                stats: {
                  ...data.stats,
            ...
            console.error(`Initial sessions API Error: ${errorText}`);
          }
        } catch (error) {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const response = await fetch(`/api/admins/${adminId}/sessions`);
  ...
  if (!admin) {
    return <Box p={2}>Admin not found</Box>;
  }
