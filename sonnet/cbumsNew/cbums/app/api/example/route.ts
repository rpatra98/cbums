 ...
 */
async function resourceHandler(req: NextRequest, context?: { params: Record<string, string> }) {
  try {
    if (req.method === "GET") {
...
export const PUT = withAuth(
  withUpdateLogging(resourceHandler, {
    resourceType: "EXAMPLE_RESOURCE",
    getResourceId: (req, res) => "existing-resource-id",
    getDetails: async (req, res) => {
      try {
        // Get the request body to include in the activity log
        const reqClone = req.clone();
...
export const DELETE = withAuth(
  withDeleteLogging(resourceHandler, {
    resourceType: "EXAMPLE_RESOURCE",
    getResourceId: (req, res) => "deleted-resource-id",
    getDetails: (req, res) => {
      return {
        method: "DELETE",
