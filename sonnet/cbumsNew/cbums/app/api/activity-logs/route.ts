  ...
  try {
              ...
              AND: [
                ...
                },
                {
                  ...
                  userAgent: deviceType === 'mobile' 
                    ...
                    : { not: { contains: ['mobile', 'android', 'iphone', 'ipad', 'ipod', 'blackberry'], mode: 'insensitive' } }
                }
              ]
            }
          ]
    ...
    if (result && result.logs) {
      console.log(`Returning ${result.logs.length} logs. Types:`, 
        [...new Set(result.logs.map((log: any) => log.action))].join(', '));
      
      // Check specifically for login/logout activities
      const authActivities = result.logs.filter((log: any) => 
