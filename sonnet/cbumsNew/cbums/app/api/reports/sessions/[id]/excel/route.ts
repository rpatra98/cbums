  ...
  async (req: NextRequest, context?: { params: Record<string, string> }) => {
    try {
      ...
      for (const field of tripFields) {
        const value = (finalTripDetails as any)[field.key];
        const displayValue = value !== undefined && value !== null 
          ? (typeof value === 'object' ? JSON.stringify(value) : String(value)) 
          : 'N/A';
        
        const isFromSample = !tripDetails || tripDetails[field.key] === undefined;
        
        tripSheet.addRow({
          field: field.label,
          value: displayValue,
          source: isFromSample ? 'SAMPLE DATA' : 'REAL DATA'
        });
        
        // Color rows from sample data differently
        const row = tripSheet.getRow(currentRow);
        if (isFromSample) {
          row.fill = {
            type: 'pattern',
            pattern: 'solid',
        ...
        currentRow++;
        console.log(`[EXCEL REPORT] Added field ${field.key}: ${displayValue}`);
      }
      
      // Add any additional fields not in our standard list
      const additionalFields = Object.entries(finalTripDetails as Record<string, any>)
        .filter(([key]) => !tripFields.some(field => field.key === key));
      
      if (additionalFields.length > 0) {
        // Add a header for additional fields
        tripSheet.mergeCells(`A${currentRow}:C${currentRow}`);
        const additionalHeader = tripSheet.getCell(`A${currentRow}`);
        additionalHeader.value = 'ADDITIONAL CUSTOM FIELDS';
        additionalHeader.font = { bold: true };
        additionalHeader.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FF7030A0' }  // Purple
        };
        additionalHeader.font.color = { argb: 'FFFFFFFF' };
        ...
        for (const [key, value] of additionalFields) {
          ...
          const isFromSample = !tripDetails || tripDetails[key] === undefined;
          ...
          currentRow++;
          console.log(`[EXCEL REPORT] Added additional field ${key}: ${displayValue}`);
        }
      }
